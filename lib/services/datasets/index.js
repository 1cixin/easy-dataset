  ...
  try {
    ...
    if (chunk.fileId && !idDistill) {
      try {
        ...
        useEnhancedPrompt = activeGaPairs.length > 0;
        logger.info(
          `检查到 ${activeGaPairs.length} 个激活的GA pairs，${useEnhancedPrompt ? '使用' : '不使用'}增强提示词`
        );
      } catch (error) {
    ...
    if (idDistill) {
      ...
      // 转换GA pairs格式以适配提示词模板
      const formattedGaPairs = activeGaPairs.map(ga => ({
        genre: `${ga.genreTitle}: ${ga.genreDesc}`,
        audience: `${ga.audienceTitle}: ${ga.audienceDesc}`,
        active: ga.isActive
      }));
    ...
    datasets.chunkContent = chunkData.content; // 保存文本块的实际内容
